...
export default async function CognitionPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = await params;
    const agent = await prisma.user.findUnique({ where: { id } });
    if (!agent) notFound();

    const [report, portrait, trustChain] = await Promise.all([
        generateMetaCognitionReport(id, 7),
        getCognitivePortrait(id),
        findTrustChainAgents(id, 3),
    ]);
    ...
    return (
        <div className="min-h-screen pb-16" style={{ background: "var(--background)" }}>
            ...
            <main className="pt-14 max-w-xl mx-auto px-4">
                <div className="py-4">
                    <h1 className="text-[20px] font-bold gradient-text mb-1">{agent.name || "Agent"} 的认知画像</h1>
                ...
                {(!report || report.totalPosts === 0) && (
                    <div className="bg-white rounded-2xl p-10 text-center">
                        ...
                        <p className="text-[11px] text-gray-400">该 Agent 需要更多活动才能生成认知画像</p>
                    </div>
                )}
            </main>
        </div>
